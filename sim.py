...
from . import surveillance_systems as cvss
from . import covid_testing as ct
from . import immunity as cvimm
from . import analysis as cva
from . import watches as cvw
from . import testing_model as cvt
from . import behaviour_model as cvbm
...
class Sim(cvb.BaseSim):
    ...
    **Examples**::
    ...
    def __init__(self, pars=None, datafile=None, label=None, simfile=None,
        ...
        # If the user has enabled the behaviour module
        if (pars is not None) and ('enable_behaviour' in pars.keys()) and (pars['enable_behaviour']):
            # Manually update behaviour pars
            pars['behaviour_pars'] = self.process_behaviour_pars(default_pars['behaviour_pars'], pars['behaviour_pars'])
    ...
    def init_people(self, popdict=None, init_infections=False, reset=False, verbose=None, **kwargs):
        ...
        elif (isinstance(self.popdict, cvppl.People)): #input is Popfile or People (infection.People)
    ...
    def init_surveillance(self): 
        if self._orig_pars and 'surveillance' in self._orig_pars:
            self['surveillance'] = self._orig_pars.pop('surveillance') # Restore

        for surveillance in self['surveillance']:
            if isinstance(surveillance, cvss.Surveillance):
                surveillance.initialize(self)

    def finalize_surveillance(self):
        for surveillance in self['surveillance']:
            if isinstance(surveillance, cvss.Surveillance):
                surveillance.finalize(self)
    ...
    def init_test_scheduler(self):
        '''
        Body: Initialize the test scheduler.
        '''
        self.test_scheduler = None
        
        pass
    
    def init_behaviour_updater(self):
        '''
        Body: Initialize the behaviour updater.
        '''
        self.behaviour_updater = cvbm.BehaviourUpdater() # Init with default parameters for now.
    ...
    def step(self):  
       ...
       # print(self.results[0]['n_infectious'])
        ...
        if self.pars['enable_testobjs']:   
            ...
            if t == 0: 
                # setattr(self, 'symptom_generator', symptoms.Symptom_generator())
                setattr(self, 'symptom_generator', symptoms.SymptomGenerator_ArrBased())
                setattr(self, 'test_case', [])
                if not(self['prev_COVID'] is None): 
                    self.symptom_generator.prev_COVID = self['prev_COVID']
                if not(self['prev_ILI'] is None):
                    self.symptom_generator.prev_ILI = self['prev_ILI']
                self.symptom_generator.prev_COVID = {symp:prev*self['symp_prev_multiplier'] for symp, prev in self.symptom_generator.prev_COVID.items()}
                self.symptom_generator.prev_ILI = {symp:prev*self['symp_prev_multiplier'] for symp, prev in self.symptom_generator.prev_ILI.items()}

            # Assign symptoms (e.g. fever, cough), including management of recoveries
            self.symptom_generator.assign_symptoms(self)
            ...
            for i, testobj in enumerate(self['testing']):
                ...
                # Interface with Covasim quarantine behaviour. Only register PCR.
                # if testobj.name == 'PCR':
    ...
    def process_behaviour_pars(self, default_behaviour_pars, passed_behaviour_pars):
        ...
        # For all behaviour parameters we want to update
        for key in passed_behaviour_pars.keys():
            if not(key in options): 
                raise RuntimeError(f'Behaviour parameter dictionary specifies a non-existent behaviour parameter. Options are {options}')
            
            # Convert dates of policy changes into integer format if they were passed as strings
            for date_key in list(passed_behaviour_pars[key].keys()):
                passed_behaviour_pars[key][self.day(date_key)] = passed_behaviour_pars[key].pop(date_key)

            # Update dictionary of policies
            default_behaviour_pars[key].update(passed_behaviour_pars[key])

        return default_behaviour_pars

    def process_multireg_pars(self):
        reg_params = self.pars['multiregion']['reg_params']
        # extract a list of names, starts, and sizes for the regions.
        rnames = []
        rsizes = []
        rstarts = []

        for rind in range(len(reg_params)): # keys are integers.
            regpars = reg_params[rind]
            rnames.append(regpars['name'])
            rsizes.append(regpars['n'])
            rstarts.append(regpars['base_uid'])
